...
class HRRR_URMA_Dataset_AllVars_CODOMAIN(Dataset):
    def __init__(self, 
                 is_train=True,
                 months=[1,12],  
                 hours="all", 
    ...
    def get_var_filepath(self, var_name, is_pred=True):
        ...
        if var_name == "terrain":
            ...
            if self.is_train:
                if is_pred:
                    data_save_path = os.path.join(path_root,"Regridded_HRRR_train_test", f"train_hrrr_alltimes_{var_name}_f{str(self.forecast_lead_time).zfill(2)}.grib2")
                else:
                    data_save_path = os.path.join(path_root,"URMA_train_test", f"train_urma_alltimes_{var_name}.grib2")
            else:
                if is_pred:
                    data_save_path = os.path.join(path_root,"Regridded_HRRR_train_test", f"test_hrrr_alltimes_{var_name}_f{str(self.forecast_lead_time).zfill(2)}.grib2")
                else:
    ...
    def process_variable(self, var_name, is_pred=True):
        ...
        xr_dataset = xr.open_dataarray(data_save_path, decode_timedelta=True, engine="cfgrib")
