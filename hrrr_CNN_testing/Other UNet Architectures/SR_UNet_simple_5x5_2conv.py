...
class SR_UNet_simple_5x5_2conv(nn.Module): #Only dealing with temperature input and output for now
    def __init__(self, 
                 n_channels_in=1, #change if inputting more channels (e.g. terrain)
                 n_channels_firstlayer=64, 
                 n_channels_out=1
                ): 
        super(SR_UNet_simple_5x5_2conv, self).__init__()
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = nn.Sequential(
                        nn.Conv2d(in_channels, 
                                  in_channels, #Note this first one needs to have same channel # in & out 
                                  kernel_size=5,
                                  stride=1, 
