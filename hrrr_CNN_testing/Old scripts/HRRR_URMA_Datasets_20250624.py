...
class HRRR_URMA_Dataset_AllTimes_AnyDates_AnyTerrains(Dataset):
    def __init__(self, 
        ...
        if is_train:
            data_save_path_pred = os.path.join(path_root,"Regridded_HRRR_train_test", f"train_hrrr_alltimes_t2m_f{str(forecast_lead_time).zfill(2)}_{RADIUS}.nc")
            data_save_path_targ = os.path.join(path_root,"URMA_train_test", f"train_urma_alltimes_t2m.nc")
        else:
            data_save_path_pred = os.path.join(path_root,"Regridded_HRRR_train_test", f"test_hrrr_alltimes_t2m_f{str(forecast_lead_time).zfill(2)}_{RADIUS}.nc")
        ...
        for i, hr in enumerate(self.hours):
            ...
            if print_flag: #for debugging
                print(f"[{i+1}/{len(self.hours)}] | Normalization for hour {hr}'s data done")
