...
class HRRR_URMA_Dataset_AllTimes_AnyDates_AnyTerrains(Dataset):
    def __init__(self, 
        ...
        list_of_idx_years = [] #stores the year of each valid sample_idx so we can do chopping later
        for i, date in enumerate(self.xr_dataset_pred.valid_time.data):
            ...
            if ((date_as_dt.month >= self.months[0])&(date_as_dt.month <= self.months[1]))&(date_as_dt.hour in self.hours):
                ...
                list_of_idx_years.append(date_as_dt.year)
        valid_date_idxs = np.array(does_date_fulfill_conditions)
        valid_date_years = np.array(list_of_idx_years)
        ...
        self.dataset_pred_normed = np.empty(np.shape(self.xr_dataset_pred[valid_date_idxs])) #haven't yet restricted xr_dataset_pred/targ to valid indices yet due to how selection below works
        self.dataset_targ_normed = np.empty(np.shape(self.xr_dataset_targ[valid_date_idxs]))
        ...
        start = time.time()
        if is_train: #need to do year-based selection, then concatenate
            tmp_dataset_pred_data = np.concatenate((self.xr_dataset_pred[valid_date_idxs[np.where(valid_date_years==2021)]].data, 
                                                    self.xr_dataset_pred[valid_date_idxs[np.where(valid_date_years==2022)]].data, 
                                                    self.xr_dataset_pred[valid_date_idxs[np.where(valid_date_years==2023)]].data))
        else: #Should only have 2024 data, so no need to get fancy. Should modify in the future if the test set expands
            tmp_dataset_pred_data = self.xr_dataset_pred.data
        print(f"Predictor dataset data loaded. Time taken = {(time.time()- start):.1f} sec")

        print(f"Loading target dataset ({year_str}, months = {self.months[0]} to {self.months[1]}, hours = {self.hours})")
        start = time.time()
        if is_train: #need to do year-based selection, then concatenate
            tmp_dataset_targ_data = np.concatenate((self.xr_dataset_targ[valid_date_idxs[np.where(valid_date_years==2021)]].data,
                                                    self.xr_dataset_targ[valid_date_idxs[np.where(valid_date_years==2022)]].data, 
                                                    self.xr_dataset_targ[valid_date_idxs[np.where(valid_date_years==2023)]].data))
        else: #Should only have 2024 data, so no need to get fancy. Should modify in the future if the test set expands
        ...
        # Now and only now, restrict the datasets, so idx works as a selector
