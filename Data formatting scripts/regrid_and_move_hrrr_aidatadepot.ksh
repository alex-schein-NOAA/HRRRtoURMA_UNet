        ...
        if [[ "${file}" == *"wrfnat"* ]]; then 
            if [[ ! "${file}" == *"idx" ]]; then #make sure it's not trying to do anything with the index files, if they exist
        		...
        		if ! test -f ${REGRID_PATH}/${yyyymmdd}/${newfilename}; then #hasn't been regridded yet - do it
        			...
        			wgrib2 ${REGRID_PATH}/${tmpfilename} -set_radius 1:6371200 -set_grib_type c3 -set_bitmap 0 -new_grid_winds grid -new_grid_interpolation bilinear -new_grid ncep grid 184 ${REGRID_PATH}/${yyyymmdd}/${newfilename}
